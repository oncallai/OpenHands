...
class Incident:
    ...
    - Iteration history for progressive analysis
    """
    ...
    # Track iteration history for progressive analysis
    iteration_analyses: List[Dict[str, Any]] = field(default_factory=list)
    iteration_plans: List[List[TroubleshootingStep]] = field(default_factory=list)
    ...
    def save_iteration_data(self) -> None:
        """Save current analysis and plan to iteration history."""
        if self.analysis:  # Only save if we have analysis data
            self.iteration_analyses.append(copy.deepcopy(self.analysis))
            self.iteration_plans.append(copy.deepcopy(self.troubleshooting_plan))
    
    def get_previous_iterations_summary(self) -> str:
        """Get a summary of previous iterations for the LLM prompt."""
        if not self.iteration_analyses:
            return "No previous iterations."
            
        result = "Previous iterations summary:\n\n"
        for i, (analysis, plan) in enumerate(zip(self.iteration_analyses, self.iteration_plans)):
            result += f"Iteration {i+1}:\n"
            result += f"Analysis: {analysis.get('summary', 'No summary')}\n"
            result += "Steps completed:\n"
            for step in plan:
                if step.completed:
                    result += f"- {step.id}. {step.description}\n"
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "iteration_history": {
                "analyses": self.iteration_analyses,
                "plans": [[step.to_dict() for step in plan] for plan in self.iteration_plans]
            }
        }
